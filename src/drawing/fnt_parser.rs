...
use gdnative::api::{file::File, visual_server::TextureFlags};

use crate::{core::error::DataError, systems::visual_server::texture::Texture};
...
impl FntParser {
    pub fn read_file(&self, path: String) -> Result<FntFile, DataError> {
        let file = File::new();
        ...
        match image_result {
            Ok(image) => {
                ...
                Result::Ok(FntFile {
                    texture: Texture::allocate(image, TextureFlags(0)),
                    text,
                })
            }
            Err(message) => {
